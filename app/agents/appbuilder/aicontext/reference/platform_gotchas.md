...
- Hours-long jobs: chain multiple short sessions, refreshing between each.
...
`session.context`. Putting a multi-MB payload in `session.context` bloats
every subsequent turn's serialized session.

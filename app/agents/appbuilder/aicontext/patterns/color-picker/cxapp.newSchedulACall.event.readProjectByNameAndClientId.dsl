FUNCTION readProjectByNameAndClientId
    LOGIC
    ...
    "operator": "EQUALS"
                    ...
                    output
                        setProjectName: UIEngine.SetStore(path = "Page.projectName.value", value = Store.urlDetails.pathParts[1]) AFTER Steps.updateFilterWithName.output
                            output
                                                                        ...
                                                                        setStore1: UIEngine.SetStore(value = Steps.insertLast1.output.result, path = "Page.query.conditions")
                                                                            output
                                                                                readPage: CoreServices.Storage.ReadPage(appCode = "rim", storageName = "Project", filter = Page.query, count = false, size = 1) AFTER Steps.setStore1.output
                                                                                    ...
                                                                                    output
                                                                                        setStore2: UIEngine.SetStore(path = "Page.project", value = Steps.readPage.output.result.content[0])
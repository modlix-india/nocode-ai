FUNCTION OTPVerfication
    NAMESPACE cxapp
    PARAMETERS
        reference AS {"type": "STRING", "version": 1}
        otp AS {"type": "STRING", "version": 1}
    EVENTS
        output
    ...
    "otp": "{{Arguments.otp}}"
                    ...
                    output
                        set: System.Context.Set(name = "Context.payload", value = Steps.make.output.value)
                            output
                                postRequest: CoreServices.REST.PostRequest(connectionName = "KYCVerification", url = "/kyc/aadhaar/okyc/otp/verify", headers = {

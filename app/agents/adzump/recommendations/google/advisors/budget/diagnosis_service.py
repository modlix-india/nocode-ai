...
Pure diagnosis logic — no API calls. Takes normalised campaign context
(from ``CampaignMetricsAdapter``) and bidding recommendation output
(from ``BiddingAdvisorService``), produces a budget constraint diagnosis
with confidence scoring.

Responsibilities:
- Classify constraint type from impression share (IS) signals.
- Guard against IS misuse on Maximize Conversions (IS unreliable).
- Compute recommended budget when constraint is budget-driven.
- Pacing warning when recommended change > 20%.
- Downgrade confidence when IS metrics are stale.
- Detect MOVE_UNUSED_BUDGET opportunities.
- Low-budget advisory when budget < 2 * tCPA (informational only).

Non-goals:
- No API calls, no hard-blocking. All outputs are confidence-scored.
- Does NOT enforce "daily budget ≥ 2 * tCPA" as a gate — the 2 * figure
  describes overdelivery, not a Smart Bidding budget rule.
  Source: https://support.google.com/google-ads/answer/1704424

Constraint classification (from IS identity):
...
Thresholds are configurable practitioner conventions (module-level constants).
...
def diagnose_budget(
    ...
    ----------
    campaign_context:
        Normalised campaign context from ``CampaignMetricsAdapter``.
    bidding_recommendation:
        Output from ``BiddingAdvisorService``, or None.
    native_budget_recs:
        Parsed native recs from ``GoogleRecommendationsAdapter`` (budget types only).
    ...
    dict
        ...
        ``recommended_budget``    — Suggested daily budget, or None.
        ...
        ``blocking_issues``       — Issue strings surfaced to the user.
        ``native_rec_amount``     — Google's suggested budget, or None.
        ``move_unused_signal``    — True when MOVE_UNUSED_BUDGET rec exists.
    """
    # Resolve the IS family by channel and gate it: Search/Shopping →
    # search_*, Display → content_*, PMax/others → None (no meaningful lost-IS,
    # so the constraint classifies as NONE).
    caps = get_capabilities(campaign_context.get("campaign_type", "UNKNOWN"))
    if not caps.supports_is_constraint_diagnosis:
        budget_lost_is = None
        rank_lost_is = None
    elif caps.is_metric_family == ISMetricFamily.CONTENT:
        budget_lost_is = campaign_context.get("content_budget_lost_impression_share")
        rank_lost_is = campaign_context.get("content_rank_lost_impression_share")
    else:

...
async def upload_and_analyze(
    ...
    filename = f"{kind}_{uuid4().hex[:12]}.{ext}"
...
async def rehost_image(
    ...
    try:
        ...
        async with httpx.AsyncClient(
            timeout=_REHOST_TIMEOUT_S, follow_redirects=True
        ) as client:
            resp = await client.get(source_url)
            if resp.status_code != 200:
                logger.info(
                    "rehost_skip: status=%d url=%s", resp.status_code, source_url[:200]
                )
                return None
            ...
            if not looks_like_image_response(raw_ctype, source_url):
                ...
                return None
            ctype = (
                (raw_ctype or _guess_ctype_from_url(source_url))
                .lower()
                .split(";", 1)[0]
                .strip()
            )
            ...
            if not data or len(data) > _REHOST_MAX_BYTES:
                logger.info(
                    "rehost_skip: size=%d url=%s", len(data or b""), source_url[:200]
                )
                return None
    except Exception as e:
        logger.info(
            "rehost_fetch_failed: url=%s err=%s", source_url[:200], str(e)[:200]
        )
        return None

...
def _next_action(cctx: CampaignContext) -> list[str]:
    ...
    scope = get_business_scope(cctx.product)
    requires_location = cctx.is_real_estate or scope in (
        "hyperlocal",
        "local",
        "regional",
    )

    if requires_location and not cctx.spec.get("location"):
        if scope in ("hyperlocal", "local") or cctx.is_real_estate:
            if cctx.pending_location:
                ...
                missing.append(
                    f"location — call `confirm_location()` ({scope or 'real-estate'})"
                )
        else:
            # Regional campaigns: bypass maps but suggest setting the scraped location immediately.
            from app.agents.adzump.tools.location import _detected_location

            detected = _detected_location(cctx.product)
            if detected:
                missing.append(
                    f"location — scraped location is **'{detected}'**. "
                    f'Call `set_campaign_spec(location="{detected}")` to confirm.'
                )
            else:
                missing.append(
                    "location — ask the user for their home region or city, then call "
                    "`set_campaign_spec(location=<what they said>)`."
                )

...
They live together so they cannot drift; ``test_keyword_prompts.py`` pins both.
"""
...
class KeywordTheme:
    ...
    # Each must agree with the guidance above — same policy, enforced in code.
...
GENERIC = KeywordTheme(
    ...
    select_guidance=_SELECT_GENERIC,
    negative_guidance=_NEGATIVES_GENERIC,
    keep_zero_volume=False,  # "drop 0-volume terms — a generic phrase nobody searches is a dead seed"
    allows_cross_business=True,
...
BRAND = KeywordTheme(
    ...
    select_guidance=_SELECT_BRAND,
    negative_guidance=_NEGATIVES_BRAND,
    keep_zero_volume=True,  # "own these even at low or zero volume (brand protection)"
    allows_cross_business=False,
    requires_brand_token=True,
    allows_informational_sources=False,
)

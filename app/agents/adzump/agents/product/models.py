...
class StorageFilter(BaseModel):
    """Simple key-value filter for storage reads."""
...
class FilterCondition(BaseModel):
    """A single filter condition with operator support."""
...
class ComplexCondition(BaseModel):
    """Grouped filter conditions (AND/OR)."""
...
class StorageReadRequest(BaseModel):
    """Standardized request for reading from nocode-saas storage."""
    ...
    eager: bool = Field(
        default=False, description="Whether to eagerly load related data"
    )
    eagerFields: List[str] = Field(
        default_factory=list, description="List of fields to eagerly load"
    )
    filter: Optional[Union[StorageFilter, ComplexCondition, FilterCondition]] = Field(
